		...
		<div class="cc_catG_left dz-details">
			...
			<p{if !$image} class="cc_catG_disabled"{/if}>
				...
				<select name="type" {if !$image}disabled{/if}>
					<option value="0"{if $image.options.type == 0} selected="selected"{/if}>Rechts</option>
					<option value="1"{if $image.options.type == 1} selected="selected"{/if}>Links</option>
					<option value="2"{if $image.options.type == 2} selected="selected"{/if}>Mittig</option>
				</select>
			</p>
		</div>
