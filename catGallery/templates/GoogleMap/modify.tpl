...
<div class="cc_catG_form" id="cc_catG_{$gallery_id}">
	...
	<div class="cc_catG_settings">
		<ul class="cc_catG_nav fc_br_left" id="cc_catG_nav_{$gallery_id}">
			<li class="fc_br_topleft">{translate('Upload new images')}</li>
			<li class="active fc_br_bottomleft">{translate('Options for frontend')}</li>
		</ul>
		<ul class="cc_catG_tabs fc_br_right">
			<li class="cc_catG_tab">{include(../default/modify/set_dropzone.tpl)}</li>
			<li class="cc_catG_tab active">{include(modify/set_frontend.tpl)}</li>
		</ul>
		<div class="clear"></div>
	</div>
	<p class="catG_IMG_y">{translate('Existing maps')}</p>
	<p class="catG_IMG_n">{translate('No map available')}</p>
