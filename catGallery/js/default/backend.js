  ...
  if (typeof catGalIDs !== "undefined" && typeof catGalLoaded === "undefined") {
    ...
    $.each(catGalIDs, function (index, cGID) {
      var $catGal = $("#cc_catG_" + cGID.gallery_id),
        ...
        $prevTemp = $(".prevTemp_" + cGID.gallery_id)
          .clone()
      ...
      $catGal
        .find(".cc_toggle_set, .cc_catG_skin input:reset")
        .unbind()
        .click(function () {
      ...
      /**/
      $imgUL.on("click", ".icon-remove", function () {
        $(this).closest("div").children("p").slideToggle(100);
      });

      $imgUL.on("click", ".cG_publish", function () {
        var $cur = $(this),
          $li = $cur.closest("li"),
      ...
      $imgUL.on("click", ".cc_catG_del_conf", function () {
        var $cur = $(this),
          $li = $cur.closest("li"),
          $inputs = $li.find("input"),
          ajaxData = {
      ...
      $imgUL.on("click", ".cc_catG_del_res", function () {
        $(this).closest("div").children("p").slideUp(100);
      });

      $imgUL.on("click", ".toggleWYSIWYG", function (e) {
        e.preventDefault();
          ...
          $.ajax({
            ...
            success: function (data, textStatus, jqXHR) {
              if (data.success === true) {
                ...
                $WYSIWYG.fadeIn(400);
                CKEDITOR.instances["wysiwyg_" + ajaxData.section_id].setData(
                  data.image.image_content
                );
                CKEDITOR.instances[
                  "wysiwyg_" + ajaxData.section_id
                ].updateElement();
                return_success(jqXHR.process, data.message);
              } else {
      ...
      dialog_form(
        ...
        function ($form, options) {
          CKEDITOR.instances[
            "wysiwygContent_" + cGID.section_id
          ].updateElement();
        }
      );
