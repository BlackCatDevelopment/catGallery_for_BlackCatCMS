...
if ($val->sanitizePost("gallery_id", "numeric") && is_object($catGallery)) {
    ...
    switch ($action) {
        ...
        case "getContent":
            ...
            break;
        case "saveContent":
            $catGallery->saveContent(
                $imgID,
                $val->sanitizePost("wysiwyg_" . $section_id, false, true)
            );
            $ajax_return = [
        ...
        case "saveIMG":
            ...
            if ($image_options != "") {
                foreach (
                    array_filter(explode(",", $image_options))
                    as $option
                ) {
                    if (
                        !$catGallery->saveImgOptions(
                            $imgID,
                            $option,
            ...
            if ($options != "") {
                foreach (array_filter(explode(",", $options)) as $option) {
                    if ($option == "wysiwygContent") {
                        $catGallery->saveOptions(
                            $option,
                            $val->sanitizePost(
                                "wysiwygContent_" . $section_id,
                                false,
                                true
                            )
                        );
        ...
        case "publishIMG":
            ...
            $ajax_return = [
                "message" => $success
                    ? $lang->translate("Image published successfully!")
                    : $lang->translate("Image unpublished successfully!"),
                "published" => $success,
                "success" => true,
            ];
            break;
